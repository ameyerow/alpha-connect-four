...
class Controller(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def handle_events(self, env: AdversarialEnv) -> Tuple[ControlType, int]:
        """
...
class ComputerController(Controller):
    def __init__(self, model: torch.nn.Module, env: AdversarialEnv):
        self.model = model
        self.mcts = MCTS(env, self.model, num_simulations=800)
    ...
    def handle_events(self, env: AdversarialEnv) -> Tuple[ControlType, int]:
        ...
        self.mcts.run()
        action_probs = self.mcts.pi()
        action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
        env.step(action)

        if env.is_terminal_state():
            return ControlType.Restart, action
        else:
            return ControlType.Player, action
...
class PlayerController(Controller):
    ...
    def handle_events(self, env: AdversarialEnv) -> Tuple[ControlType, int]:
        ...
        if new_control_type is None:
            return ControlType.Player, None
        else:
            return new_control_type, action
...
def main():
    ...
    controllers[ControlType.Computer] = ComputerController(model, env)
    controllers[ControlType.Restart] = RestartController()
    
    while True:
        controller: Controller = controllers[active_control_type]
        active_control_type, action = controller.handle_events(env)
        if action is not None:
            controllers[ControlType.Computer].mcts.step(action)
        render(env, active_control_type, screen)
